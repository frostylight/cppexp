...
inline auto min(const T1 &x, const T2 &y) {
    return x > y ? y : x;
}

template<typename T1, typename T2>
inline auto max(const T1 &x, const T2 &y) {
    return x > y ? x : y;
}
...
playerBullet::playerBullet(cREAL &X, cREAL &Y, cREAL &dX, cREAL &dY, cint &Damage)
...
enemyBullet::enemyBullet(cREAL &X, cREAL &Y, cREAL &dX, cREAL &dY, cint &Damage)
  : bullet(X, Y, dX, dY, Damage){};
...
enemy::enemy(cREAL &X, cREAL &Y, cREAL &sX, cREAL &sY, cint &sTime, cint &Health)
  : chara(X, Y, Health), sx(sX), sy(sY), lx(X), ly(Y), stayTime(sTime) {
    rx = (rand() % 51 - 25) * 0.025;
    ry = (rand() % 25 - 25) * 0.01;
}
...
void enemy::update() {
    ...
    if(!f) {
        x += fround((sx - x) * 0.025);
        y += fround((sy - y) * 0.025);
        if(abs(sx - x) < 20 && abs(sy - y) < 24)
            f = 1;
    } else {
        if(!stayTime) {
            x += fround((lx - x) * 0.025);
            y += fround((ly - y) * 0.025);
            if(abs(lx - x) < 10 && abs(ly - y) < 12)
                disable();
        } else {
            --stayTime;
            x += rx;
